...
/-!
Pre-compilation draft for the F3 arithmetic-codec pilot.
...
This source is the reviewed 27-source pilot.  Its only contact with the
historical literal is one private definitional normalization; all public
comparison results pass through arithmetic generation and saturation.
-/

namespace CollatzClassical
namespace KL2003
namespace F3CoreArithmeticCodecPilot
...
theorem formulaUnrank_rank (e : FormulaEdge) :
  ...
  cases e with
  | retarded i =>
      simp [formulaRank, formulaUnrank]
  | advancedDirect i ell =>
      have hi := (directSourceRank i).2
      have he := ell.2
      have hsplit := Nat.div_add_mod
        (3 * (directSourceRank i).1 + ell.1) 3
      simp [formulaRank, formulaUnrank]
      omega
  | advancedParityLift i ell =>
      have hi := (liftSourceRank i).2
...
theorem formulaRank_injective : Function.Injective formulaRank :=
  formulaEdgeEquivFin729.injective
...
theorem pilotUnrank_rank (e : PilotFormulaEdge) :
  ...
  cases e with
  | retarded i => simp [pilotRank, pilotUnrank]
  | advancedDirect k ell =>
      have hk := k.2
      have he := ell.2
      have hsplit := Nat.div_add_mod (3 * k.1 + ell.1) 3
      simp [pilotRank, pilotUnrank]
      omega
  | advancedParityLift k ell =>
      have hk := k.2
      have he := ell.2
...
theorem pilotPositionToRank_frozenPos (e : PilotFormulaEdge) :
  ...
  | retarded i =>
      ...
      simp only [pilotFrozenPos, pilotPositionToRank, pilotRank]
      split_ifs <;> omega
  | advancedDirect k ell =>
      have hk := k.2
      have he := ell.2
...
-/
private theorem pilotFrozen_position_normalization :
...
def pilotFrozenIndex (e : PilotFormulaEdge) : Fin pilotFrozen.length :=
  Fin.cast pilotFrozen_length.symm (pilotFrozenPos e)

theorem pilotFrozenIndex_uses_frozenPos (e : PilotFormulaEdge) :
    (pilotFrozenIndex e).1 = (frozenPos (pilotEmbed e)).1 := by
  simpa [pilotFrozenIndex] using pilotFrozenPos_agrees e

private theorem pilot_position_at_formula (e : PilotFormulaEdge) :
...
theorem pilot_frozen_at_formula (e : PilotFormulaEdge) :
    pilotFrozen.get (pilotFrozenIndex e) = pilotRealize e := by
  rw [pilotFrozen_position_normalization]
  simp [pilotFrozenIndex, pilotPositionRealize,
    pilotPositionUnrank_frozenPos]

theorem pilotRealize_mem_frozen (e : PilotFormulaEdge) :
    pilotRealize e ∈ pilotFrozen := by
  rw [← pilot_frozen_at_formula e]
  exact List.get_mem pilotFrozen (pilotFrozenIndex e)
...
theorem pilot_frozen_scope (edge : CoreEdge) (h : edge ∈ pilotFrozen) :
  ...
  obtain ⟨j, rfl⟩ := hm
  cases pilotUnrank j <;>
    simp [pilotRealize, realize, pilotEmbed, formulaSource,
      fin27To243, directSourceUnrank, directSourceUnrankNat,
      liftSourceUnrank, liftSourceUnrankNat, fin9To81] <;> omega
...
theorem matrixOf_eq_of_perm {left right : List CoreEdge}
  ...
  exact (h.filter (edgeMatches s t)).foldr_eq'
    (fun _ _ _ _ _ => by dsimp; ac_rfl) 0
...
end F3CoreArithmeticCodecPilot
end KL2003
