...
/-!
Post-elaboration environmental coverage.  Unlike the 151-entry explicit-source
inventory above, this walks every constant whose fully qualified name lies in
the repaired module namespace, including constructors, recursors,
no-confusion helpers, and deriving implementations generated by elaboration.
The execution-side log checker requires one profile line per reported
declaration and rejects `Lean.ofReduceBool` or `sorryAx` in any profile.
-/

run_meta do
  let env ← Lean.getEnv
  let namespacePrefix := `CollatzClassical.KL2003.F3CoreArithmeticCodecPilotRepair
