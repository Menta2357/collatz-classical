...
/-!
Compact extraction of one coherent infinite source branch.

The scheduler-depth module supplies selected provenances of arbitrarily large
finite length.  This module first applies the inverse-system form of Konig's
lemma to their homogeneous action-list serializations.  The resulting family
contains one selected prefix at every exact length and is coherent under
`List.take`.
-/

namespace GeneralKInfiniteBranchExtraction
...
def IsSelectedCodePrefix {p : Nat} {hp : 1 <= p}
    ...
    (hne : NeverStops initial) (code : List (PackedSourceAction p)) : Prop :=
  exists time : Nat,
    code <+: sourceWalkActionList (selectedOccurrence hne time).target.walk

/-- Selected code prefixes at one exact depth. -/
abbrev SelectedCodeAtDepth {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    (hne : NeverStops initial) (depth : Nat) :=
...
theorem nonempty_selectedCodeAtDepth {p : Nat} {hp : 1 <= p}
  ...
  let fullCode := sourceWalkActionList (selectedOccurrence hne time).target.walk
  ...
  have hdepth : depth <= fullCode.length := by
    ...
    omega
  refine ⟨⟨code, ?_, ?_⟩⟩
  · simpa [code, List.length_take, Nat.min_eq_left hdepth]
...
def prefixProjection {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    {hne : NeverStops initial} {i j : Nat} (hij : i <= j)
...
theorem prefixProjection_trans {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    {hne : NeverStops initial} {i j k : Nat}
...
theorem selectedCodeAtDepth_chainCoherent {p : Nat} {hp : 1 <= p}
  ...
  exact (sourceWalkActionList_chainCoherent
    (selectedOccurrence hne time).target.walk).of_prefix hprefix
...
noncomputable def coherentPackedAction {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    (hne : NeverStops initial) (n : Nat) : PackedSourceAction p :=
...
noncomputable def coherentPackedStream {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    (hne : NeverStops initial) : Stream' (PackedSourceAction p) :=
...
noncomputable def extractedInfiniteSourceWalk {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    (hne : NeverStops initial) : GeneralKInfiniteBranchDescent.InfiniteSourceWalk hp where
...
theorem extractedInfiniteSourceWalk_segment_code {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    (hne : NeverStops initial) (start length : Nat) :
    sourceWalkActionList
...
theorem extractedInfiniteSourceWalk_prefix_code {p : Nat} {hp : 1 <= p}
    {root : ELLabel (p + 1)} {initial : GeneralKSourceGenealogy.ProvenancedTree hp root}
    (hne : NeverStops initial) (length : Nat) :
    sourceWalkActionList
...
end GeneralKInfiniteBranchExtraction
