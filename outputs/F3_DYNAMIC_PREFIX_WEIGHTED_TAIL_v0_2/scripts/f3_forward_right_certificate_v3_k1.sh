...
source "$repo/outputs/F3_DYNAMIC_PREFIX_WEIGHTED_TAIL_v0_2/scripts/f3_forward_right_certificate_v3_common.sh"

result_dir="$repo/outputs/F3_DYNAMIC_PREFIX_WEIGHTED_TAIL_v0_2/results/F3_FORWARD_RIGHT_CERTIFICATE_V3"
...
test "$(unique_value F3_FORWARD_V3_C0_LEAN_EXIT_STATUS "$compile_log")" = '0'
test "$(unique_value F3_FORWARD_V3_C0_WRAPPER_EXIT_STATUS "$compile_log")" = '0'
test "$(unique_value F3_FORWARD_V3_A1 "$audit_log")" = 'PASS'
test "$(unique_value F3_FORWARD_V3_A1_LEAN_EXIT_STATUS "$audit_log")" = '0'
test "$(unique_value F3_FORWARD_V3_A1_WRAPPER_EXIT_STATUS "$audit_log")" = '0'
c0_head=$(unique_value F3_FORWARD_V3_C0_HEAD "$compile_log")
a1_head=$(unique_value F3_FORWARD_V3_A1_HEAD "$audit_log")
test "$c0_head" = "$a1_head"
test "$c0_head" = "$(git -C "$repo" rev-parse HEAD)"
test "$c0_head" = "$(git -C "$repo" rev-parse public/codex/hilo2-f3-forward-right-certificate-v3)"

target_olean_hash=$(unique_value F3_FORWARD_V3_TARGET_OLEAN_SHA256 "$compile_log")
target_ilean_hash=$(unique_value F3_FORWARD_V3_TARGET_ILEAN_SHA256 "$compile_log")
audit_olean_hash=$(unique_value F3_FORWARD_V3_AUDIT_OLEAN_SHA256 "$audit_log")
audit_ilean_hash=$(unique_value F3_FORWARD_V3_AUDIT_ILEAN_SHA256 "$audit_log")
...
printf '%s\n' \
  'F3_FORWARD_V3_K1=PASS' \
  'F3_FORWARD_V3_STABLE_DECLARATIONS=21' \
  'F3_FORWARD_V3_EXPLICIT_AXIOM_AUDITS=21' \
  "F3_FORWARD_V3_NAMESPACE_DECLARATIONS=$namespace_count" \
  "F3_FORWARD_V3_AXIOM_PROFILES=$profile_count" \
  "F3_FORWARD_V3_UNIQUE_PROFILE_NAMES=$unique_profile_count" \
  'F3_FORWARD_V3_STABLE_ALLOWED_AXIOM_SET_ONLY=PASS' \
  'F3_FORWARD_V3_GENERATED_ALLOWED_SET_PLUS_LCPROOF=PASS' \
  'F3_FORWARD_V3_FULL_LOG_FORBIDDEN_AXIOMS=ABSENT'
