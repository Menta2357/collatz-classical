...
test "$(unique_value FULL_CORE_IDENTITY_C0 "$c0_log")" = 'PASS'
test "$(unique_value FULL_CORE_IDENTITY_C0_LEAN_EXIT_STATUS "$c0_log")" = '0'
test "$(unique_value FULL_CORE_IDENTITY_C0_WRAPPER_EXIT_STATUS "$c0_log")" = '0'
test "$(unique_value FULL_CORE_IDENTITY_C0_SOURCE_SHA256 "$c0_log")" = "$source_hash"
test "$(unique_value FULL_CORE_IDENTITY_C0_EXACT_CORE_SHA256 "$c0_log")" = "$exact_hash"
test "$(unique_value FULL_CORE_IDENTITY_C0_REPAIR_SHA256 "$c0_log")" = "$repair_hash"
test "$(unique_value FULL_CORE_IDENTITY_C0_DECODER_SHA256 "$c0_log")" = "$decoder_hash"
c0_identity_olean_hash=$(unique_value FULL_CORE_IDENTITY_OLEAN_SHA256 "$c0_log")
c0_identity_ilean_hash=$(unique_value FULL_CORE_IDENTITY_ILEAN_SHA256 "$c0_log")
...
printf '%s\n' 'FULL_CORE_IDENTITY_A1_PREFLIGHT=PASS'
printf 'FULL_CORE_IDENTITY_A1_SOURCE_SHA256=%s\n' "$source_hash"
printf 'FULL_CORE_IDENTITY_A1_AUDIT_SOURCE_SHA256=%s\n' "$audit_hash"
printf 'FULL_CORE_IDENTITY_INPUT_OLEAN_SHA256=%s\n' "$identity_olean_hash"
printf 'FULL_CORE_IDENTITY_INPUT_ILEAN_SHA256=%s\n' "$identity_ilean_hash"
...
printf 'FULL_CORE_IDENTITY_A1_LEAN_EXIT_STATUS=%s\n' "$lean_status"
...
printf '%s\n' \
  'FULL_CORE_IDENTITY_A1=PASS' \
  'FULL_CORE_IDENTITY_OVERLAY_FILE_COUNT_AFTER_A1=7' \
  'FULL_CORE_IDENTITY_OVERLAY_SYMLINK_COUNT_AFTER_A1=0'
printf 'FULL_CORE_IDENTITY_AUDIT_OLEAN_SHA256=%s\n' \
  "$(shasum -a 256 "$audit_olean" | awk '{print $1}')"
printf 'FULL_CORE_IDENTITY_AUDIT_ILEAN_SHA256=%s\n' \
